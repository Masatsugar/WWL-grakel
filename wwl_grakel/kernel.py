...
def to_ndarray(wl_feat: List[Dict[int, str]]) -> np.ndarray:
...
class WassersteinWeisfeilerLehman:
    def __init__(
        self,
        n_iter: int = 2,
        sinkhorn: bool = True,
    ...
    def compute_wasserstein_distance(self, G):
        ...
        for h in range(0, self.n_iter + 1):
            M = np.zeros((n, n))
            # Iterate over pairs of graphs
            for graph_index_1, graph_1 in enumerate(tqdm(label_sequences)):
                ...
                for graph_index_2, graph_2 in enumerate(
                    ...
                    costs = scipy.spatial.distance.cdist(
                        labels_1, labels_2, metric=self.ground_distance
                    )
